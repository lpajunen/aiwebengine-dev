...
function serveEditor(context) {
  ...
  // Require authentication. The auth context is always present, so it is
  // isAuthenticated — not the context itself — that tells an anonymous
  // visitor apart from a signed-in one.
  ...
  const isAuthorized = req.auth.isAdmin || req.auth.isEditor;
...
AUTHENTICATION:
- context.request.auth - AuthContext, always present
- context.request.auth.isAuthenticated - Whether the request is authenticated
...
- context.request.auth.isEditor - Whether user has editor privileges
- Check !context.request.auth.isAuthenticated to detect unauthenticated requests

IMPORTANT CONCEPTS:
1. Scripts are SERVER-SIDE JavaScript that handle HTTP requests
2. Use Response builders instead of manual response objects
3. Check context.request.auth.isAuthenticated to verify authentication
4. Access user data through context.request.auth (userId, userEmail, userName, isAdmin, isEditor)
